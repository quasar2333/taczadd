...
local BURST_TRACK = increment(static_track_top)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (gun_kick_state.melee_count == 1) then
            context:runAnimation("melee_1", track, true, PLAY_ONCE_STOP, 0)
            gun_kick_state.melee_count = 2
        else
            context:runAnimation("melee_2", track, true, PLAY_ONCE_STOP, 0)
            gun_kick_state.melee_count = 1
        end
        if (context:getAmmoCount() > 1) then
            context:runAnimation("burst", context:getTrack(STATIC_TRACK_LINE, BURST_TRACK), false, PLAY_ONCE_STOP, 0)
        end
    end
    return nil
end
...
function M:initialize(context)
    ...
    self.gun_kick_state.melee_count = 1
end
