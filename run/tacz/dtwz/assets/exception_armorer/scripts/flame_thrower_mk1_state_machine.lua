...
local AMMO_TRACK = increment(static_track_top)
...
local SHOOT_SOUND_TRACK = increment(blending_track_top)
...
function base_track_state.entry(this, context)
    ...
    context:runAnimation("ammo_display", context:getTrack(STATIC_TRACK_LINE, AMMO_TRACK), false, PLAY_ONCE_HOLD, 0)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, AMMO_TRACK))
end

function base_track_state.update(this, context)
    local max = context:getMaxAmmoCount()
    local now = context:getAmmoCount()
    local track = context:getTrack(STATIC_TRACK_LINE, AMMO_TRACK)
    context:resumeAnimation(track)
    context:setAnimationProgress(track, (now/max)*0.9, true)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getCurrentTimestamp() - context:getLastShootTimestamp() > 800) then
            context:runAnimation("shoot_first", (context:getTrack(BLENDING_TRACK_LINE, SHOOT_SOUND_TRACK)), true, PLAY_ONCE_STOP, 0)
        end
...
local M = {
    ...
    SHOOT_SOUND_TRACK = SHOOT_SOUND_TRACK,
