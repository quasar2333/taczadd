...
local MODE_TRACK_LINE = increment(track_line_top)
local MODE_TRACK = increment(blending_track_top)
...
function base_track_state.update(this, context)
    local track = context:findIdleTrack(MODE_TRACK_LINE, false)
    if (context:getFireMode() == SEMI) then
        if (base_track_state.mode == 1) then
            if (not context:isAiming()) then
                context:runAnimation("switch_to_semi", track, true, PLAY_ONCE_STOP, 0.2)
            end
            base_track_state.mode = 0
        end
    elseif (context:getFireMode() == BURST) then
        if (base_track_state.mode == 0) then
            if (not context:isAiming()) then
                context:runAnimation("switch_to_burst", track, true, PLAY_ONCE_STOP, 0.2)
            end
            base_track_state.mode = 1
        end
    end
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == BURST) then
            ...
            elseif (gun_kick_state.melee_light_count == 3) then
                ...
                gun_kick_state.melee_light_count = 4
            elseif (gun_kick_state.melee_light_count == 4) then
                context:runAnimation("melee_light_1_1", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_light_count = 1
            end
        elseif (context:getFireMode() == SEMI) then
            if (gun_kick_state.melee_heavy_count == 0) then
                context:runAnimation("melee_heavy_1", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_heavy_count = 1
            elseif (gun_kick_state.melee_heavy_count == 1) then
                context:runAnimation("melee_heavy_2", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_heavy_count = 2
            elseif (gun_kick_state.melee_heavy_count == 2) then
                context:runAnimation("melee_heavy_1_1", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_heavy_count = 1
            end
        end
    end
...
function ads_state.aimming.entry(this, context)
    ...
    context:runAnimation("aim_in", track, true, PLAY_ONCE_HOLD, 0.1)
end
...
local M = {
    ...
    MODE_TRACK_LINE = MODE_TRACK_LINE,
...
function M:initialize(context)
    ...
    context:ensureTracksAmount(MODE_TRACK_LINE, blending_track_top.value)
