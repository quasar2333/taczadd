...
local OVERLOAD_TRACK = increment(static_track_top)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getAmmoCount() == 2) then
            ...
            gun_kick_state.overload = 1
        else
            if (gun_kick_state.overload == 1) then
                context:runAnimation("shoot_1", track, true, PLAY_ONCE_STOP, 0)
                gun_kick_state.overload = 0
            elseif (gun_kick_state.overload == 2) then
                context:runAnimation("shoot_2", track, true, PLAY_ONCE_STOP, 0)
                gun_kick_state.overload = 0
            elseif (gun_kick_state.overload == 3) then
                context:runAnimation("shoot_3", track, true, PLAY_ONCE_STOP, 0)
                gun_kick_state.overload = 0
            else
                context:runAnimation("shoot_1", track, true, PLAY_ONCE_STOP, 0)
                gun_kick_state.overload = 0
...
local function runReloadAnimation(context)
    ...
    if (gun_kick_state.overload == 0) then
        context:runAnimation("reload", track, false, PLAY_ONCE_STOP, 0.2)
    elseif (gun_kick_state.overload == 1) then
        context:runAnimation("overload_1", track, false, PLAY_ONCE_STOP, 0.2)
    elseif (gun_kick_state.overload == 2) then
        context:runAnimation("overload_2", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
function reload_state.entry(this, context)
    print("overload", gun_kick_state.overload)
    if (gun_kick_state.overload ~= 3) then
        runReloadAnimation(context)
        reload_state.timestamp = context:getCurrentTimestamp()
    end
end

function reload_state.update(this, context)
    if (gun_kick_state.overload == 0 or gun_kick_state.overload == 3) then
        context:trigger(this.INPUT_RELOAD_FINISH)
    end
    if (gun_kick_state.overload ~= 0 and gun_kick_state.overload ~= 3) then
        if (reload_state.timestamp ~= -1 and context:getCurrentTimestamp() - reload_state.timestamp > 330) then
            gun_kick_state.overload = gun_kick_state.overload + 1
            reload_state.timestamp = -1
            context:trigger(this.INPUT_RELOAD_FINISH)
        end
    end
end
...
function reload_state.transition(this, context, input)
    if (input == this.INPUT_RELOAD_FINISH) then
...
function base_track_state.entry(this, context)
    ...
    context:runAnimation("overload_display", context:getTrack(STATIC_TRACK_LINE, OVERLOAD_TRACK), false, PLAY_ONCE_HOLD, 0)
end

function base_track_state.update(this, context)
    context:runAnimation("static_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
    local track = context:getTrack(STATIC_TRACK_LINE, OVERLOAD_TRACK)
    if(gun_kick_state.overload <= 1)then
        context:setAnimationProgress(track, 0.25, false)
    elseif(gun_kick_state.overload == 2)then
        context:setAnimationProgress(track, 1.25, false)
    elseif(gun_kick_state.overload == 3)then
        context:setAnimationProgress(track, 2.25, false)
    end
end
...
local M = {
    ...
    INPUT_RELOAD_FINISH = "input_reload_finish",
...
function M:initialize(context)
    ...
    self.reload_state.timestamp = -1
    self.gun_kick_state.overload = 1
end
