...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BAYONET_STOCK) then
        context:runAnimation("melee", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.idle
    end
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        --local num = (getRandom(600)+3) % 5 + 1
        --if (num == gun_kick_state.shoot_count) then
            --num = (num + 3) % 5 + 1
        --end
        --gun_kick_state.shoot_count = num
        local animationName = "shoot_" .. tostring((gun_kick_state.shoot_count + 1) % 6 +1 )
        context:runAnimation(animationName, track, true, PLAY_ONCE_STOP, 0)
        gun_kick_state.shoot_count = gun_kick_state.shoot_count + 1
    end
    return nil
end
