...
local function isNoAmmo(context)
    return (not context:hasBulletInBarrel())
end
...
function base_track_state.entry(this, context)
    ...
    context:runAnimation("ammo_display", context:getTrack(STATIC_TRACK_LINE, AMMO_TRACK), false, LOOP, 0)
...
function base_track_state.update(this, context)
    ...
    context:setAnimationProgress(track,max(context:getAmmoCount() + 0.25, 0.25),false)
...
function bolt_caught_states.bolt_caught.entry(this, context)
    context:runAnimation("barrel_empty", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, LOOP, 0)
end
