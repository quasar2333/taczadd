...
local function runInspectAnimation(context)
    ...
    if (context:getAmmoCount() <= 1) then
        ...
        context:runAnimation("inspect_charged", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        elseif (gun_kick_state.melee_light_count == 2) then
            ...
            if (context:getAmmoCount() > 1) then
                context:runAnimation("charge_2", fire_track, false, PLAY_ONCE_STOP, 0)
            end
...
function bolt_caught_states.entry(this, context)
    this.bolt_caught_states.update(this, context)
end

function bolt_caught_states.update(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK)
    if (context:getAmmoCount() <= 1) then
        context:runAnimation("static_idle", track, false, PLAY_ONCE_HOLD, 0)
    else
        context:runAnimation("charge_idle", track, false, PLAY_ONCE_HOLD, 0)
    end
end
...
function main_track_states.start.transition(this, context, input)
    if (input == INPUT_DRAW) then
        if (context:getAmmoCount() <= 1) then
            ...
            context:runAnimation("draw_charged", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
        end
