...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == SEMI) then
            -- 初始化射速计数器
            if (context:getShootInterval() >= 600) then
            ...
            if (gun_kick_state.melee_count == 0) then
                context:runAnimation("melee_semi_1", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_count = 1
            elseif (gun_kick_state.melee_count == 1) then
                context:runAnimation("melee_semi_2", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_count = 2
            elseif (gun_kick_state.melee_count == 2) then
                context:runAnimation("melee_semi_3", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_count = 3
            elseif (gun_kick_state.melee_count == 3) then
                context:runAnimation("melee_semi_4", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_count = 4
            elseif (gun_kick_state.melee_count == 4) then
                context:runAnimation("melee_semi_1_1", track, true, PLAY_ONCE_STOP, 0.1)
                gun_kick_state.melee_count = 1
            end
        end
    end
