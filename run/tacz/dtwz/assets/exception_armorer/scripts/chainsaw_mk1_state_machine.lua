...
local SAW_TRACK = increment(static_track_top)
...
function shoot_state.entry(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, SAW_TRACK)
    context:runAnimation("shoot_loop", track, false, PLAY_ONCE_STOP, 0)
end

function shoot_state.update(this, context)
    local lastShoot = context:getLastShootTimestamp()
    local nowTime = context:getCurrentTimestamp()
    local shootInterval = context:getShootInterval()
    if (this.shoot_state.shooting == 1) then
        local track = context:getTrack(STATIC_TRACK_LINE, SAW_TRACK)
        if (context:isStopped(track)) then
            context:runAnimation("shoot_loop", track, false, PLAY_ONCE_STOP, 0)
        end
        if (nowTime - lastShoot > shootInterval + 200) then
            local track_1 = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
            context:runAnimation("shoot_end", track_1, true, PLAY_ONCE_STOP, 0)
            this.shoot_state.shooting = 0
        end
    end
end
