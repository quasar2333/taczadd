...
local AMMO_TRACK = increment(blending_track_top)
local IDLE_TRACK = increment(blending_track_top)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        if (context:getAttachment("GRIP") ~= "tacz:empty") then
            ...
            local ammoCount = context:getAmmoCount()
            if (ammoCount == 6) then
                context:runAnimation("shoot_1", track, true, PLAY_ONCE_STOP, 0)
            elseif (ammoCount == 5) then
            ...
            elseif (ammoCount == 3) then
                context:runAnimation("shoot_4", track, true, PLAY_ONCE_STOP, 0)
            elseif (ammoCount == 2) then
                context:runAnimation("shoot_5", track, true, PLAY_ONCE_STOP, 0)
            elseif (ammoCount == 1) then
                context:runAnimation("shoot_6", track, true, PLAY_ONCE_STOP, 0)
            end
            -- 锁死射速为 120
...
function ammo_state.entry(this, context)
    context:runAnimation("ammo_display", context:getTrack(BLENDING_TRACK_LINE, AMMO_TRACK), true, LOOP, 0)
    context:pauseAnimation(context:getTrack(BLENDING_TRACK_LINE, AMMO_TRACK))
end

function ammo_state.update(this, context)
    local track = context:getTrack(BLENDING_TRACK_LINE, AMMO_TRACK)
    context:resumeAnimation(track)
    context:setAnimationProgress(track, context:getAmmoCount() * 0.25 + 0.1, false)
...
function movement_track_states.idle.update(this, context)
    local track = context:getTrack(BLENDING_TRACK_LINE, IDLE_TRACK)
