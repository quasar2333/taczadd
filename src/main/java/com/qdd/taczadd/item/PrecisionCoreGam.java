...
public class PrecisionCoreGam extends GamItem{
    private int min =0;
    private int max =1;
    public PrecisionCoreGam(Properties properties,int min,int max) {
        this(properties);
        this.min=min;
        this.max=max;
    }
    public PrecisionCoreGam(Properties properties) {
    ...
    public Pair<String,Float> getEffct(ItemStack stack) {
        return new Pair<>("CRT",stack.getOrCreateTag().getFloat("effect"));
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        ...
        Arrays.stream(Component.translatable("tooltip.gam.precisioncore", string).getString().split("\n")).forEach(
