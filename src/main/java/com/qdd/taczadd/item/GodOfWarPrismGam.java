...
public class GodOfWarPrismGam extends GamItem{
    private int min =0;
    private int max =1;
    public GodOfWarPrismGam(Properties properties,int min,int max) {
        this(properties);
        this.min=min;
        this.max=max;
    }
    public GodOfWarPrismGam(Properties properties) {
    ...
    public Pair<String,Float> getEffct(ItemStack stack) {
        return new Pair<>("CTA",stack.getOrCreateTag().getFloat("effect"));
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        ...
        Arrays.stream(Component.translatable("tooltip.gam.godofwarprism", string).getString().split("\n")).forEach(
