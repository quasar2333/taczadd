...
public class RoyalGiftGam extends GamItem{
    public RoyalGiftGam(Properties properties) {
    ...
    public List<Pair<String,Float>> applygam(ItemStack stack, Long num){
        ...
        if (num>=2){
            list.add(new Pair<>("armor_m", 0.5f));
        }
        return list;
    }
    @Override
    public Pair<String,Float> getEffct(ItemStack stack) {
        return new Pair<>("armor_a",stack.getOrCreateTag().getFloat("effect"));
    }

    @Override
    public Float randomEffect(){
        return new Random().nextInt(20,50)/10f;
    }

    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        super.appendHoverText(stack, level, list, flag);
        String string=stack.getOrCreateTag().getFloat("effect")==0?"???":stack.getOrCreateTag().getFloat("effect")+"";
        Arrays.stream(Component.translatable("tooltip.gam.royal_gift", string).getString().split("\n")).forEach(
