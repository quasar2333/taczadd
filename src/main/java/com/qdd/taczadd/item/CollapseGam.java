...
public class CollapseGam extends GamItem{
    private int min =0;
    private int max =1;
    public CollapseGam(Properties properties,int min,int max) {
        this(properties);
        this.min=min;
        this.max=max;
    }

    public CollapseGam(Properties properties) {
    ...
    public List<Pair<String,Float>> applygam(ItemStack stack, Long num){
        ...
        if (num==4){
            list.add(new Pair<>("armorIgnore", 0.2f));
        }
        if (num>=2){
            list.add(new Pair<>("damageModifier", 0.1f));
        }
        return list;
    }
    ...
    public Pair<String,Float> getEffct(ItemStack stack) {
        return new Pair<>("armorIgnore",stack.getOrCreateTag().getFloat("effect"));
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        ...
        Arrays.stream(Component.translatable("tooltip.gam.collapse", string).getString().split("\n")).forEach(
