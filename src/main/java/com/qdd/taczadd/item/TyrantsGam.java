...
public class TyrantsGam extends GamItem{
    private int min =0;
    private int max =1;
    public TyrantsGam(Properties properties,int min,int max){
        this(properties);
        this.min=min;
        this.max=max;
    }
    public TyrantsGam(Properties properties){
    ...
    public List<Pair<String,Float>> applygam(ItemStack stack, Long num){
        ...
        if (num>=2){
            list.add(new Pair<>("rpmgam", 0.05f));
        }
        return list;
    }
    ...
    public Pair<String,Float> getEffct(ItemStack stack) {
        return new Pair<>("damageModifier",stack.getOrCreateTag().getFloat("effect"));
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        ...
        Arrays.stream(Component.translatable("tooltip.gam.tyrants", string).getString().split("\n")).forEach(
